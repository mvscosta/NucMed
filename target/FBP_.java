...
public class FBP_ extends javax.swing.JFrame implements
	...
	public static int janela_ativa = TELA_RAMPA; // Variavel da janela
	 ...
	 * 
	 * @param img :
	 *            recebe a imagem do sinograma selecionada para a reconstrucao
	 */
	...
	private boolean AbrirImg() {
		...
		while (repetir) {
			setVisible(false);
			imp = AbrirImagem.Abrir_FBP(IJ.getInstance(), debugAtivo);
			if (imp == null)
	...
	private void Reconstruir(java.awt.event.ActionEvent evt) {
		...
		if (AbrirImg()) {
			...
			imagemReconstruida.show();
			IJ.showTime(imp, startTime, " Tempo de Execução do FBP ");
		}
		// selecao_rampa ();
	}
	 ...
	 * 
	 * @param img :
	 *            imagem informada do sinograma para reconstrução
	 ...
	 *            Parâmetro tratado pelo JAVA.
	 * @param tf :
	 *            Parâmetro informa o componente selecionado.
	 */
	 ...
	 *            Parâmetro informa o tipo de janela para alteração. Para
	 *            variável tipo_parametro podemos ter as seguintes entradas: 0 =
	 *            Configura tela para retroprojeção simples 1 = configura tela
	...
	/**
	 * Função que retorna como vetor do tipo Complexo o filtro para a
	 * reconstrução.
	 * 
	 * @param rampa
	...
	private Complexo[] gerar_filtro(boolean rampa, int tipo_filtro) {
		...
		Filtros filtros = new Filtros(Float.parseFloat(tfParametro1.getText()),
				Float.parseFloat(tfParametro2.getText()), tipo_filtro,
				tamanho_imagem);
	...
	/**
	 * Procedimento utilizado para criar e exibir a janela contendo o gráfico
	...
	/**
	 * Função utilizada para fazer a filtragem da imagem selecionada antes da
	 * reconstrução.
	 * 
	 * @param img :
