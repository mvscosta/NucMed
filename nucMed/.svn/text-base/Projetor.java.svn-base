	...
	public void view0(float[] mapa, float[] imgpixels) {
		...
		for (x = -xmax; x < xmax; x++) {
			...
			for (int y = -ymax; y <= ymax; y++) {
				...
				s = (int) Math.floor(x + 0.5);
				// int p1 =Funcoes.mudaCoord(view, s, true);
				// int p2 =Funcoes.mudaCoord(y, s, false);
				int p1 = Funcoes.mudaCoord(view, s, true, xmax, dim);
				int p2 = Funcoes.mudaCoord(y, s, false, xmax, dim);
	...
	public static void view270(float mapa[], float img[]) {
		...
		for (x = -xmax; x < xmax; x++) {
			...
			for (int y = -ymax; y <= ymax; y++) {
				...
				/*
				 * A array de float "sinopixels" estava recebendo o comando
				 * comentado a seguir sinopixels[p1] += W * imgpixels[p2];
				 * 
				 * Creio que esta variavel deveria receber o comando na
				 * sequencia Substituiu-se "imgpixels" por "img", ja que este
				 * metodo recebe como parametro "img" e nao "imgpixels" *
				 */
	...
	public static void quadrante1(float[] mapa, float[] imgpixels) {
		...
		for (view = 1; view < view90; view++) {
			...
			for (x = (float) (-xmax + 0.625); x <= (float) (xmax - 0.625); x += 0.25) {
				...
				/*
				 * 
				 * A prof. faz as verificacoes seguintes, ja a Michele NaO, pq?
				 * ay[0]=(Y1f==Y2f)?-1:(ymin-Y1f)*Iddy12;
				 * ax[0]=(X1f==X2f)?-1:(X1f-xmin)*Iddx12;
				 * 
				 */
				...
				while (a < 1) {
					...
					try {
						if (ax[ix] < ay[iy - 1]) {
							...
							sinopixels[p1] += W * imgpixels[p2];
							// System.out.println(sinopixels[p1]);
							a = ay[iy++];
							Y++;
						...
						a = 1;
						IJ.log("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
					}
